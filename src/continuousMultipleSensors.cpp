...
void sigintHandler(int) { exitFlag = 1; }

int main() {
  // Configuration constants
  // Number of sensors. If changed, make sure to adjust pins and addresses
  // accordingly (ie to match size).
  const int SENSOR_COUNT = 3;
  // GPIO pins to use for sensors' XSHUT. As exported by WiringPi.
  const uint8_t pins[SENSOR_COUNT] = {25, 27, 21};
  // Sensors' addresses that will be set and used. These have to be unique.
  const uint8_t addresses[SENSOR_COUNT] = {VL53L0X_ADDRESS_DEFAULT + 2,
  ...
  // For each sensor: create object, init the sensor (ensures power on), set
  // timeout and address Note: don't power off - it will reset the address to
  // default!
  for (int i = 0; !exitFlag && i < SENSOR_COUNT; ++i) {
    try {
      // Initialize...
      ...
      try {
        ...
        sensors[i]->setVcselPulsePeriod(VcselPeriodPreRange, 18);
        sensors[i]->setVcselPulsePeriod(VcselPeriodFinalRange, 14);
      } catch (const std::exception &error) {
        std::cerr << "Error enabling long range mode with reason:" << std::endl
                  << error.what() << std::endl;
        return 2;
      }
      ...
      std::cerr << "Error initializing sensor " << i
                << " with reason:" << std::endl
                << error.what() << std::endl;
      return 1;
    }
  }
  ...
  for (int i = 0; !exitFlag && i < SENSOR_COUNT; ++i) {
    try {
      ...
      std::cerr << "Error starting continuous read mode for sensor " << i
                << " with reason:" << std::endl
                << error.what() << std::endl;
      return 2;
    }
  ...
  for (; !exitFlag && j < 100000; ++j) {
    ...
    for (int i = 0; !exitFlag && i < SENSOR_COUNT; ++i) {
      ...
      try {
        ...
        std::cerr << std::endl
                  << "Error geating measurement from sensor " << i
                  << " with reason:" << std::endl
                  << error.what() << std::endl;
        // You may want to bail out here, depending on your application - error
        // means issues on I2C bus read/write. return 3;
        distance = 8096;
      }
    ...
    // Skip comparing first measurement against max and min as it's not a full
    // iteration
    if (j == 0) {
      continue;
    }
  ...
  std::cout << "Avg frequency: " << 1000000000 / (totalDuration / (j + 1))
            << "Hz" << std::endl;
